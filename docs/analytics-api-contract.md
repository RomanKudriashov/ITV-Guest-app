...
**Разбор параметров отказывает, а не подставляет своё.** Неразбираемая дата —
`422 bad_date` (поле `date_from`/`date_to`), неизвестный пресет — `422 bad_preset`,
`date_from` позже `date_to` — `422 bad_range`, неизвестная гранулярность —
`422 bad_granularity`. Пустой параметр — это «не прислали», у него остаётся
умолчание. Молча взять другой период значило бы отдать 200 с цифрами за не те
сутки, и заметить это можно было бы только сверив эхо периода в ответе.

**Часы — настоящие.** `granularity=hour` режет по часам в поясе отеля; бакет —
`2026-07-20T14:00` (местное время, без пояса в строке). Час заказа — час его
СОЗДАНИЯ, как и сутки относят все меры к дате создания: иначе заказ, сделанный
в 23:50 и завершённый в 00:10, попал бы в разные столбики двух разрезов одного
дашборда. Часы читаются из журнала `AnalyticsEvent` (суточная строка роллапа
часов не содержит) и потому ограничены: не больше **31 суток** за раз, иначе
`422 range_too_large`. Месячный пресет помещается целиком.
...
`analytics_order_daily` копит выручку РАЗЛОЖЕННО из снимка `charges` заказа:
`revenue_minor` — только позиции (subtotal), плюс отдельные меры
`service_fee_minor`, `delivery_minor`, `tax_minor`, `tip_minor`. `summary.current`
несёт эти поля и `gross_minor` (сумма всего). Разложение считает тот же
редьюсер по снимку сырого факта — recompute == live. Заказы без снимка
считаются как «всё в позициях», компоненты по нулям.

## Разрез по категории номера

`dimension=room_category` — разбивка по тарифной категории; `room_category=<id>`
— фильтр, `room_category=none` — заказы без категории (пустую строку в
query-параметре не отличить от «фильтр не задан»).

**Категория пишется СНИМКОМ в момент заказа** (`room_category_key` в слепке и в
`OrderDaily`), а не резолвится по комнате. Иначе перевод комнаты из «Стандарта»
в «Делюкс» переписал бы прошлые отчёты: март менялся бы в июне.

**Строки без категории не прячутся.** Под пустым ключом лежат и номера без
категории, и заказы, созданные до появления разреза; бэкфилла нет и быть не
может — на момент заказа категории не существовало, а нынешняя категория
комнаты это НЕ то, что продавали тогда. Спрятав эти строки, мы получили бы
сумму долей, не сходящуюся с итогом.

**Сравнение категорий между собой.** Абсолютные числа несравнимы: люксов
восемь, стандартов девяносто. Поэтому строки этого разреза несут ещё три поля:

```jsonc
{"key": "...", "label": "Люкс", "orders": 6,
 "rooms": 2,                // номеров в категории СЕЙЧАС (историю это не знает)
 "orders_per_room": 3.0,
 "ratio_to_base": 3.0, "base_label": "Стандарт"}
```

`ratio_to_base` считается к самой слабой категории — чтобы фраза звучала как
«люкс заказывает в 3 раза чаще стандарта», а не «в 0,33 раза реже». Строка без
категории в сравнении не участвует (`orders_per_room: null`), но из выдачи не
исчезает.
