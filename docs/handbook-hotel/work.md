...
Наследование порога работало всегда, а вот увидеть картину целиком было негде:
приходилось открывать точки по одной. Теперь на экране заведений есть список
**всех** точек — с их порогом, умолчанием вида работы и тем, какое число
применяется на самом деле.

Показываются все, а не только переопределённые: без знаменателя «переопределено
три» ни о чём не говорит.

Смотреть туда стоит не из любопытства. Порог красит заказы просрочкой,
просрочка поднимает эскалацию, эскалация будит старшего. Точка с порогом в пять
минут, поставленным когда-то на время, разбудит его сегодня ночью — и никто не
...
Вкладка «События» — что система сообщает сотрудникам. Из коробки включено
только то, после чего нужно действовать: заявку никто не взял, заявку
отменили, гость поставил низкую оценку, уведомление не дошло до канала.
Сообщения гостя в чат и события оформления отель включает сам. У каждого
события можно выбрать, кому и какими каналами слать, и написать свой текст.
Рядом — «вот так придёт»: текст собран на последнем настоящем случае отеля и
показан каждому получателю на его языке. Если такого случая ещё не было,
экран так и говорит — выдуманного примера не показывает.

Недоставленное уведомление приходит в общий канал отеля (без отдела и без
сотрудника). Без такого канала о сломанном канале узнать некому.
