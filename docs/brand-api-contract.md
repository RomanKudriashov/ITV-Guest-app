...
и `GET /api/v1/guest/*` отдают `hotel.theme` из `BrandTheme` отеля. Этот контракт
...
Художественное направление: **тёмная база, насыщенный тёмно-синий акцент**,
плотная крупная типографика, глубина. Зелёный убран из акцента (остаётся только
как статус `success`). Дисплейный шрифт — **Prata**, интерфейсный — **Manrope**
(обе с кириллицей).

### Пресеты

Добавлены тёмно-синие пресеты (существующие v1 остаются как опции):

| Код | Образ |
|---|---|
| `midnight_navy` | Тёмная база, синий акцент, Prata — **сигнатурный пресет** |
| `sapphire_dark` | Индиго и сталь, тёмный |
| `porcelain_navy` | Светлый, синий акцент |
| `harbor_light` | Светлый день у воды, глубокий синий |

Как и раньше, пресет **вычисляется из 4–5 опорных цветов** на режим
(`brand_palette.build_color_set`), а не задаётся 18 значениями руками.

### Производные токены (на клиенте)

Базовый набор палитры — **ровно 18 токенов на режим** (бэкенд, тесты пинят это
число). Богатый словарь редизайна — **сильный акцент, мягкая заливка, свечение
акцента, третий уровень текста, шкала скруглений и шкала глубины** — выводится
из этих 18 в слое темы (`createAppTheme`) чистой цветовой математикой и
доступен компонентам как `theme.palette.brand.*`. Ни одного захардкоженного
цвета: производные — функции токенов, а не новые значения. Хранимый набор
остаётся 18.

### Подложка-картинка

Оператор грузит свою картинку тем же медиапайплайном, что и логотип
(`POST /api/v1/cms/media`, `kind: brand`), и кладёт url в
`brand.background.imageUrl` с `kind: "image"`. Затемнение — токеном
`background.dim` (одна картинка на обе темы).

### Правило

**Эмодзи не используются** — ни в интерфейсе, ни в данных/сиде. Иконки —
монохромный набор компонентов, наследующих цвет текста (красятся токеном).
