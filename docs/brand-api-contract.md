...
Сохранённые токены уже отражаются в гостевом ответе: `POST /api/guest/session`
и `GET /api/guest/*` отдают `hotel.theme` из `BrandTheme` отеля. Этот прогон
...
### `POST /api/cms/brand/apply-preset`
...
### `GET /api/cms/brand/abstractions`
...
Через существующий медиапайплайн (`POST /api/cms/media`, `kind=brand`).
