...
**Есть ли каталог (`has_catalog`, волна 9, пункт 10).** Отдел без меню и
коммерции — ресепшен: гость пишет ему в чат, он оформляет заказы за гостя и
передаёт задачи. У такого заведения:
* разделы не заводятся — `POST /cms/categories` → `422 service_without_catalog`;
* своей коммерции нет — PATCH полей коммерции → `422 service_without_catalog`;
* на витрине и в поиске гостя его нет, даже если раздел завели мимо экрана;
* расписание, персонал, SLA, эскалация — как у всех.

Меняет признак только администратор отеля. Снять каталог с заведения, где есть
активные разделы, нельзя — `422 service_has_catalog_content`: меню не прячется
молча. Ресепшен новых отелей заводится без каталога; у существующих признак
снят миграцией там, где разделов не было (28 на стенде).
