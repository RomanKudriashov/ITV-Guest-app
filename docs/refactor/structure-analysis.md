...
## Найдено по ходу партий, чинить отдельно

### `ItemDetailOut` объявлен дважды (нашла партия 1)

`apps/catalog/schemas/guest.py` и `apps/catalog/schemas/cms.py` содержат разные классы с одним
именем — так было и до переноса (`api/schemas.py` и `api/cms/schemas.py`). В рантайме это не
видно: каждая вьюха сериализует своим классом. А вот в OpenAPI компонент называется по имени
класса, поэтому в схеме остаётся ОДИН `ItemDetailOut` — и половина эндпоинтов документирована
чужим телом. Кто читает схему (генератор клиента, интеграция), получает неверный контракт.

Почему не починили в партии 1: лечится переименованием, а партия переименований не делает —
переезд и переименование в одном коммите при разборе поломки не разделить. Отдельной правкой:
переименовать гостевой в `GuestItemDetailOut`, сверить OpenAPI, поправить фронт, если он
генерирует типы из схемы.

Заодно это доказало, что сверка OpenAPI по ТЕЛАМ, а не только по путям, нужна: при переносе
`api/cms/catalog.py` импортировал одноимённый класс из гостевого модуля. Пути совпадали,
тела — нет; сверка тел это поймала.

## Что сделала партия 2 (каталог)

`apps/catalog` разложен по слоям и забрал свои эндпоинты:

- `models/` — 20 моделей в восьми файлах по ресурсам (category, item, modifier, facets,
  badge, routing, slots, inclusions). Импорт остался прежним, `apps.catalog.models`:
  43 файла не должны знать, в каком модуле лежит класс. Таблицы, `app_label` и миграции
  не тронуты — `makemigrations --check` молчит.
- `services/` — восемь модулей: `menu` (бывший `services.py`), `cms` (бывший
  `cms_services.py`), `availability`, `showcase`, `search`, `home`, `inclusions`, `slots`.
  Реэкспорта в `__init__` намеренно нет: импорт обязан называть модуль, иначе через
  полгода снова получится свалка, только с приставкой `services`.
- `api/guest/` — витрина, карточка, слоты, главная, поиск. `api/cms/` — категории,
  позиции, модификаторы, справочники. Сборка снизу вверх в `api/router.py`, наружу домен
  отдаёт два роутера.

Настройки уровня отеля (быстрые действия, главная, поиск, витрина, коммерция) уехали не в
каталог, а в `apps/hotels`: они правят `Hotel.settings`, `Hotel.latitude` и `ShowcaseTile`.
Каталог остался тем, кто по этим настройкам СОБИРАЕТ витрину.

**Отклонение от плана партии.** План клал плитки витрины в `apps/catalog/api/cms/showcase.py`.
Положили в `apps/hotels/api/cms/settings_showcase.py`: и модель (`ShowcaseTile`), и порог
группировки (`Hotel.showcase_group_threshold`), и калитка доступа — отеля. Иначе вьюха
каталога ходила бы в модель отеля, а это ровно тот стоп-сигнал, ради которого партии и режут.

### `ItemDetailOut` разведён

Гостевая схема переименована в `GuestItemDetailOut`; CMS-схема имя сохранила. В OpenAPI
появился один новый компонент, тела ответов не изменились — в классе поменялось только имя.
