...
## Что сделала партия 7 (тесты)

### Правило раскладки

Тест лежит там, где живёт ЕГО ПРЕДМЕТ — модуль, чьё поведение он проверяет. Слой выбирается так
же, как в коде: идёт через HTTP — `tests/{app}/api/`, зовёт функции напрямую — `tests/{app}/services/`.
Тест, задевающий несколько приложений (а таких большинство: гость почти всегда проходит через
сессию, каталог и заказ), кладётся к тому приложению, НА ЧЬИ ОТВЕТЫ ОН СМОТРИТ, а не к тому, чьи
модели ему понадобились по дороге.

### Что осталось в корне и почему

`conftest.py` и `helpers.py` — общие: фикстуры видны всем, кто лежит ниже. Дробить их по
приложениям значит рисковать тем, ради чего партия и делалась осторожно: конфтест определяет
видимость фикстуры, и тест, потерявший её при переезде, не падает, а ПРОПУСКАЕТСЯ.

`test_safety_net.py` — тоже в корне: эта сеть натянута над всеми приложениями сразу и не является
частью ни одного из них.

`grms_harness.py` уехал в `tests/grms/` — помощник одного домена.

### Что правилось помимо перемещения

Ровно то, что ломается от переезда, и ничего больше:

1. **Относительные импорты.** `from .conftest import host_for` из `tests/catalog/api/` указывает уже
   не туда. Заменены на абсолютные `from tests.conftest import ...`.
2. **Пути, посчитанные от `__file__`.** `Path(__file__).resolve().parents[2] / "docs"` — арифметика,
   верная ровно на одной глубине вложенности. Заведены `TESTS_DIR`, `FIXTURES`, `REPO_ROOT`
   в `tests/helpers.py`: корень теперь один и переезд его не двигает.

Ни один тест не переписан, ни одна проверка не ослаблена.

### Параллельный прогон (та же партия)

Полтора часа на прогон стоили дорого весь рефакторинг, поэтому после переноса включён `pytest-xdist`
в четыре процесса: **1:22:52 → 0:29:05**, почти втрое.

Риск был назван заранее: тесты писались под один процесс и могли конфликтовать по базе. Не
конфликтуют — `pytest-django` заводит каждому воркеру свою базу с суффиксом имени, и в чужую никто
не пишет.

Проверялось НЕ итоговой строкой, а построчно, по junit-отчётам: у каждого из 829 тестов сверялся
исход. Три прогона — однопоточный и два параллельных — совпали полностью: ни один тест не пропал,
ни один не сменил исход, пропущенных те же шесть.

Второй параллельный прогон делался именно потому, что первый сошёлся сразу: xdist раздаёт тесты по
процессам в разном порядке, и гонка, которой повезло не проявиться однажды, проявляется во второй
раз. Одно совпадение доказательством не считается.

Отладка одного файла — с `-n 0`: четыре подготовки базы ради одного теста стоят дороже теста.
