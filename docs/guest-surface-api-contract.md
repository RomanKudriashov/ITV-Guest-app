...
| Метод | Путь |
|---|---|
| GET | `/api/v1/tracker/chat/threads?cursor=&limit=` | диалоги страницей: `{items[]{thread_id, room, last_body, last_at, unread}, next_cursor, unread_total}` |
...
Список листается курсором (свежие сверху: `last_message_at`, затем id), по 30,
не больше 100; пустые треды в него не попадают. `unread_total` — непрочитанные
по всем диалогам, для значка.
