...
```jsonc
{
  ...
  // Последний ЗАКРЫТЫЙ заказ, который гость может оценить; null — просить не о чем.
  // Одна карточка на стартовой: не по одной на заказ. Более старые неоценённые
  // следом не всплывают. Закрытие карточки — дело витрины (на устройстве гостя).
  "to_review": {"id": "...", "number": 1041, "closed_at": "2026-07-22T19:10:00+03:00",
                "summary": "Цезарь", "extra_count": 0}
}
```
