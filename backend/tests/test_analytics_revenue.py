"""
Разложение выручки: аналитика различает позиции / сервисный сбор /
доставку / налог / чаевые из снимка charges на Order.

По конвейеру: событие → record() → сырой факт → редьюсер читает ТОЛЬКО
...
def test_order_without_charges_counts_all_as_revenue():
    ...
    class _OldOrder:
        # Заказ, оформленный до коммерции: снимка нет (нули), но total есть.
