...
def test_only_actionable_events_are_on_by_default():
    ...
    assert enabled == {
        ...
        # Передача диалога поимённо: человеку отдали переписку, и он обязан
        # узнать — иначе передача превращается в то же «пусть кто-нибудь
        # заметит», от чего она и уводит. Требует действия по определению:
        # адресат должен пойти и ответить гостю.
        "chat.handover",
    }
