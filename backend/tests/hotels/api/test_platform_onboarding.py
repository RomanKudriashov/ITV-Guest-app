...
def test_seed_fills_empty_registries_once(api):
    """
    УСЛОВИЕ ЗАДАЁМ САМИ: реестр должен быть ПУСТ к началу проверки.

    Раньше пустота доставалась даром — база сеялась заново на каждую проверку.
    Теперь посев общий на воркер, и «пусто» стало свойством окружения, а не
    условием проверки: она зеленела бы или краснела в зависимости от того, что
    делала соседка. Опустошаем реестр явно — предмет проверки от этого не
    меняется: заполняет ли сид ПУСТОЙ реестр и не дублирует ли при повторе.
    """
    # ЖЁСТКО, А НЕ МЯГКО. Обычное удаление у нас помечает запись, и код остаётся
    # ЗАНЯТЫМ: сид получает 409 «код занят удалённой записью». Проверке нужен
    # по-настоящему пустой реестр, иначе она проверяет не то, что написано.
    OnboardingTemplate.all_objects.all().hard_delete()
