...
def _assert_block_matches(name: str, expected: set[str]) -> None:
    blocks = _blocks(name)
    # ПРОПАВШЕЕ ПЕРЕЧИСЛЕНИЕ — ПОЛОМКА, А НЕ ПОВОД ПРОМОЛЧАТЬ.
    #
    # Здесь стоял `pytest.skip("…пока не появилось")` — со времён, когда книгу
    # только начинали размечать. Сегодня размечены все шесть перечислений, и
    # тот же `skip` означает уже другое: удали разметку из книги — и сторож
    # книги зазеленеет ровно на том, что он один и сторожит.
    assert blocks, (
        f"перечисления «{name}» нет ни в одном документе. Оно было — значит, "
        f"разметку из книги убрали, и сверять код с книгой стало нечем."
    )
