...
class MediaAsset(TenantModel):
    class Kind(models.TextChoices):
        ...
        # Файл шрифта, загруженный отелем. СВОЙ вид, а не `other`: шрифт не
        # картинка — его не режут на варианты, не считают яркость и не кадрируют,
        # и в медиатеке он не должен попадаться среди фотографий.
        FONT = "font", "Шрифт"
        # Кадр рекламного баннера. СВОЙ вид, а не `item`: иначе реклама
        # растворится в медиатеке среди фотографий блюд, и оператор, ищущий
        # свой баннер, получит свалку из трёхсот снимков еды.
        BANNER = "banner", "Рекламный баннер"
        OTHER = "other", "Прочее"
