...
# КУЛДАУН ПОСЛЕ НЕУДАЧИ — КОРОТКИЙ.
#
# Длинный кулдаун защищает провайдера от сотни одновременных обновлений, и это
# правильно — но только ПОСЛЕ УДАЧНОГО ответа: тогда данные свежие, и спешить
# некуда. После отказа он работал наоборот: одна кратковременная пятисотка
# гасила блок погоды на все двадцать минут, потому что снять ключ было некому.
# Так и случилось на стенде 21.09.2026 — провайдер ответил 503 в момент
# перезапуска, а через пять минут отвечал 200.
#
# Минута — компромисс, а не произвол: следующий гость получает новую попытку
# почти сразу, а лежащий по-настоящему сервис видит не больше одного запроса в
# минуту на отель вместо потока.
RETRY_AFTER_FAILURE = int(getattr(settings, "WEATHER_RETRY_SECONDS", 60))
...
def mark_refreshed(hotel_id) -> None:
    """
    Продлить кулдаун до штатного: данные свежие, ходить к провайдеру незачем.

    Зовётся ТОЛЬКО после удачного ответа. Разделение намеренное: короткий ключ
    ставится перед попыткой и сам истечёт, если попытка не удалась.
    """
    cache.set(_cooldown_key(hotel_id), "1", REFRESH_AFTER)
...
def ensure_fresh(hotel) -> None:
    ...
    Ставится он КОРОТКИМ — на минуту. Удачная попытка продлит его до штатных
    двадцати (`mark_refreshed`), неудачная не продлит ничего, и через минуту
    следующий гость получит новую попытку вместо пустого блока.
    """
    entry = cache.get(_key(hotel.pk))
    if entry:
        age = (
    ...
    if not cache.add(_cooldown_key(hotel.pk), "1", RETRY_AFTER_FAILURE):
        return
