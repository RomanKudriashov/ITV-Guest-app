"""
Уборка остатков автотестов в отеле — и в команде платформы.

ОБЛАСТЬ ШИРЕ, ЧЕМ `--subdomain`. Всё, кроме одного шага, относится к названному
отелю. Шаг с учётками платформы — флотовый: они заводятся прогонами консоли и
отеля не имеют вовсе (`hotel = NULL`). Сказано здесь потому, что «убрал у
Кристалла, а снесло платформенное» — ровно та неожиданность, которой в этом
проекте не место.
