...
class Command(BaseCommand):
    ...
    def _seed_rich_catalog(self, hotel, points, locations, schedules):
        ...
        # ВЕНЬЮ-ЧАСЫ «КРИСТАЛЛА» — КРУГЛОСУТОЧНЫЕ, И ЭТО НАРОЧНО.
        #
        # С тех пор как часы заведения стали ЗАПИРАТЬ заказ (а не только
        # рисовать пилюлю «открыто до…»), закрытая кухня гасит все свои блюда.
        # «Кристалл» — рабочий стенд: на нём гоняется гостевой e2e, и половина
        # набора заказывает еду. С часами 07:00–23:00 весь этот набор падал
        # ночью — не из-за дефекта, а из-за времени суток на стене.
        #
        # Показывать закрытое заведение есть где: «Азур» и «Люмен» (демо-флот)
        # свои часы сохранили — там и винный бар с 17:00, и экскурсии до 18:00,
        # и талассо до 21:00. Витрина «Кристалла» при этом ничего не теряет:
        # пилюля часов рисуется по тому же расписанию, а круглосуточное
        # заведение показывает «открыто».
        round_the_clock = Schedule.objects.filter(
            hotel=hotel, is_always_open=True
        ).first() or Schedule.objects.create(
            hotel=hotel, name="Круглосуточно", is_always_open=True
        )
        for code in ("kitchen", "bar", "spa", "sakura", "terrace"):
            self._set_service_schedule(code, round_the_clock)
