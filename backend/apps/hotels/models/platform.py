...
class SystemDictionaryEntry(BaseModel):
    ...
    деактивировать: справочник платформы — источник, а не поводок.

    ГРАНИЦА ДЕЙСТВИЯ, ЧЕСТНО (переписано в G-групповой партии).

    Прежняя редакция этого текста говорила, что правка достаётся отелям,
    заведённым после неё. Это было неверно даже в этой части: копии нарезал
    `provisioning.seed_item_data_dictionaries()`, и читал он КОНСТАНТЫ КОДА, а
    не эту таблицу, — то есть добавленная здесь запись не доезжала ни до кого
    вовсе, включая новых. Ещё раньше текст обещал «приезжает в старые только
    явным действием»: действия тогда не существовало.

    Как это работает СЕЙЧАС:

    * новый отель получает копии ИЗ ЭТОЙ ТАБЛИЦЫ — она единственный источник;
    * правка доезжает до тех, кто копию не трогал: совпадающая с ПРЕЖНИМ
      значением эталона копия не тронута по определению;
    * тронутая копия остаётся как есть ВСЕГДА — ни правка эталона, ни что-либо
      ещё её не перетирает;
    * расхождение видно платформе числом и по записям
      (`GET /platform/dictionaries/divergence`), а возвращается к эталону
      только явным действием оператора (`POST /platform/dictionaries/reset`).

    Ключ сопоставления — `(kind, code)`, общий у эталона и у копий (`Allergen`,
    `DietaryMarker` с `is_system`). Именно поэтому перевод на этот механизм не
    потребовал ни одной миграции данных.
    """
