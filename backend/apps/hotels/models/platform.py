...
class SystemDictionaryEntry(BaseModel):
    ...
    ГРАНИЦА ДЕЙСТВИЯ, ЧЕСТНО. Правка здесь достаётся ТОЛЬКО отелям, заведённым
    после неё. До существующих она не доезжает никогда и ничем: механизма
    распространения — ни автоматического, ни по кнопке — не существует, и
    расхождение эталона с копиями копится молча. Прежняя редакция этого текста
    обещала «приезжает в старые только явным действием» — действия не было и
    нет; фраза читалась как описание поведения, а была описанием намерения.

    Сравнить копию с эталоном при этом возможно без миграций: у `Allergen` и
    `DietaryMarker` есть `code` и `is_system`, ключ сопоставления `(kind, code)`
    общий с обеими сторонами. Пока экрана расхождений нет — считайте, что
    отель, заведённый в прошлом году, живёт с прошлогодним списком.
    """
