...
# --- Свой шрифт отеля ------------------------------------------------------
#
# Курируемый список остаётся списком: «любой шрифт из интернета» мы по-прежнему
# не принимаем. Но отель со своей типографикой существует, и до сих пор ему
# нечего было ответить, кроме «возьмите похожий».
#
# ЛИЦЕНЗИЯ — НЕ НАША. Проверить право на файл кодом невозможно; отель
# подтверждает его при загрузке, и это записано в интерфейсе словами, а не
# спрятано в согласии мелким шрифтом.

FONT_MAX_BYTES = 2 * 1024 * 1024

# Сигнатуры файлов шрифтов. Проверяем ИХ, а не Content-Type: браузеры шлют
# шрифты то как `font/woff2`, то как `application/octet-stream`, то как
# `application/font-woff2` — довериться заголовку значит пустить в хранилище
# что угодно под правильным именем.
FONT_SIGNATURES = {
...
def font_family_of(name: str) -> str:
    """
    Строка семейства для своего шрифта отеля.

    Запасные шрифты те же, что у курируемых: файл может не доехать — по сети,
    по кэшу, по блокировщику, — и витрина обязана остаться читаемой, а не
    показать гостю системный «Times» там, где верстался гротеск.
    """
