...
# СКОЛЬКО «ЧИТАЕМ СОСТОЯНИЕ…» ИМЕЕТ ПРАВО ДЛИТЬСЯ.
#
# «Читаем» — это обещание: сейчас разберёмся. Обещание без срока перестаёт быть
# обещанием: гость час смотрел на него вместо честного «нет связи», потому что
# выход из этого состояния зависел ТОЛЬКО от того, спросит ли клиент ещё раз.
# Не спросил (вкладка в фоне — мобильные браузеры усыпляют таймеры, пропала
# сеть, клиент без повторов) — и «читаем» остаётся на экране навсегда.
#
# Десять секунд. Одно чтение стоит до `READ_BUDGET_S` = 2.5 с, экран
# переспрашивает через 1.8 и 3.0 с — вторая НАСТОЯЩАЯ попытка завершается
# примерно к восьмой секунде. Десять покрывают её с запасом и при этом
# достаточно коротки, чтобы «читаем» не читалось как состояние покоя.
#
# Срок считается от ПЕРВОГО молчания, а не от последнего запроса: иначе частый
# опрос отодвигал бы честный ответ бесконечно.
READING_DEADLINE_S = 10.0
...
def _read_state(context: RoomContext) -> tuple[dict, str]:
    ...
    * не ответил ни один СНОВА → ENDPOINT_UNREACHABLE («связи нет»).
    ...
    if not successful:
        ...
        # Либо повтор подтвердил молчание, либо вышел срок «читаем»: второе
        # закрывает случай, когда повтора не будет вовсе.
        proven = (
            silent >= COLD_READ_ATTEMPTS and silent_for >= commands.READ_COALESCE_S
        ) or silent_for >= READING_DEADLINE_S
        if not proven:
...
def _unavailable(session, reason: str, *, language: str, verified: bool, context=None) -> dict:
    ...
    return {
        ...
        "room_verified": verified,
        "can_command": False,
        # РАЗМЕТКА ЕСТЬ, ЗНАЧЕНИЙ НЕТ.
        #
        # Раньше здесь стоял пустой список, и гость видел пустой экран: ни
        # плана, ни зон, ни намёка, что номер вообще оборудован. Разметка при
        # этом известна — она в опубликованном снимке и от связи не зависит.
        #
        # Значения не подставляем никакие: ни «выключено», ни последние
        # известные. Элемент без связи не имеет состояния, и показать «что
        # было» — это показать неправду. Команды заперты `can_command: False`.
        "zones": _zones_without_values(context, language=language),
        **_plan(context),
    }


def _zones_without_values(context, *, language: str) -> list[dict]:
    """
    Зоны из опубликованного снимка: названия, элементы, их вид — но без единого
    значения. Пустой список вместо этого означал бы «номер не оборудован», а он
    оборудован; неизвестно только его состояние.
    """
    if context is None:
        return []
    return _serialize_zones(context, {}, language=language)
