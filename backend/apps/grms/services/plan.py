...
def for_guest(plan: object) -> dict:
    ...
    return {
        ...
        "image": image,
        "image_off": image_off,
        # ВИД ЭКРАНА — из снимка, а не выведенный клиентом по наличию кадров.
        #
        # Гость про уровни не знает и слова этого не увидит: поле говорит
        # экрану, КАК рисовать комнату — простой план заливает включённые зоны,
        # полный показывает свет настоящим светом. Выводить это из `image_off`
        # значило бы вернуть догадку, ради устранения которой заведён уровень.
        #
        # Снимок старой версии поля не несёт — тогда падаем на прежнее правило,
        # и откат к v2 продолжает работать.
        "level": plan.get("level") or ("full" if image_off else "simple"),
