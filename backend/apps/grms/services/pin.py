...
def verify(hotel, session, *, pin: str) -> dict:
    ...
    # ПОДТВЕРЖДЕНИЕ В КОМНАТЕ ОДНО. Новое гасит прежние.
    #
    # Подтверждения копились: устройство, введшее код месяц назад, оставалось
    # подтверждённым, пока код не сменят. Между заездами код меняют не всегда —
    # и тогда телефон прошлого гостя продолжал управлять номером рядом с
    # телефоном нового.
    #
    # Гасим ПОДТВЕРЖДЕНИЕ, а не сессию: у прежнего устройства остаётся корзина
    # и история, а управление номером возвращается вводом того же кода. Для
    # второго телефона той же семьи это означает «управляет тот, кто вводил
    # последним» — цена, которую мы платим за то, чтобы не гадать, кто из двух
    # устройств чей.
    from apps.accounts.models import GuestSession

    # Внутри тенант-контекста: `GuestSession` тенантная и под RLS — без
    # выставленного тенанта запрос вернул бы НОЛЬ строк, а не ошибку, и
    # прежние подтверждения молча пережили бы новое.
    with tenant_context(hotel):
        GuestSession.objects.filter(
            room_id=session.room_id, room_verified_at__isnull=False
        ).exclude(pk=session.pk).update(room_verified_at=None)
...
def set_pin(hotel, room, *, pin: str, valid_until=None):
    ...
    Смена PIN СБРАСЫВАЕТ подтверждение у всех устройств этой комнаты: момент
    смены гостя администратор отмечает сменой кода, и прежний гость обязан
    потерять управление номером в этот момент, а не когда-нибудь.

    `valid_until` — ДАТА ВЫЕЗДА, известная ресепшену при заселении. После неё
    код не действует: `RoomPin.is_active` перестаёт быть истиной, ввод кода
    отбивается, а уже выданное подтверждение гаснет (см. `guest.room_verified`).
    Пусто — код живёт, пока его не сменят: это по-прежнему законный вариант для
    отеля, который срок выезда в панель не заносит.
    """
    from apps.accounts.models import GuestSession
    from apps.grms.models import RoomPin
