...
# ПОЛЯ, КОТОРЫЕ ЧИТАЕТ ТОЛЬКО ИНЖЕНЕР.
#
# Сырой ответ оборудования, тег обмена и длительность — это разбор на объекте:
# по ним видно, ЧТО именно сказало железо и на каком звене оборвалось.
# Администратору отеля они не нужны и вредны: он не поедет чинить коннектор, а
# «iRidi вернул status:false» прочитает как поломку приложения. Ему достаточно
# «связь есть» или «связи нет».
#
# Поля именно ВЫРЕЗАЮТСЯ на сервере, а не прячутся на экране: спрятанный, но
# приезжающий ответ — это утечка, которую видно в консоли браузера, и мы это
# уже проходили.
# РАЗБОР ОБМЕНА — ИНЖЕНЕРУ, ИТОГ — ОТЕЛЮ.
#
# Эти шесть полей отвечают на вопрос «что реально сказало железо»: их читает наш
# инженер, и по ним он едет на объект. Администратору отеля они не нужны и
# вредны — `{"status":"false"}` он прочитает как поломку приложения и позвонит
# нам вместо того, чтобы просто увидеть «не подтвердилось».
#
# Остальное (когда, номер, элемент, отправленное значение, исход и причина
# словами) отелю остаётся: это его собственная работа, а не наша.
ENGINEER_ONLY = ("raw_response", "request_id", "device", "command", "feedback", "duration_ms")


def for_hotel(row: dict) -> dict:
    """
    Строка журнала без инженерных полей — то, что видит отель.

    Режем ЗДЕСЬ, на сервере, а не на экране: спрятанное вёрсткой поле всё равно
    приезжает в браузер и видно в консоли — то есть не спрятано вовсе.
    """
    return {key: value for key, value in row.items() if key not in ENGINEER_ONLY}
