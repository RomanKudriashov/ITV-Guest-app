...
ROOM_PROFILES: dict[str, dict[str, int]] = {
    ...
    # Демо-номер гостевого экрана (G5). Это НЕ комната с боевого стенда: она
    # взята из демо-отеля, на неё завязан весь E2E, и профиль подобран под
    # демо-конфигурацию — пять групп света по зонам плана, две шторы (основная
    # и блэкаут) и один фанкойл.
    #
    # Вторая штора здесь — свойство ЭМУЛЯТОРА, а не находка на объекте: тега
    # блэкаута на боевом стенде нет (прозвон §8.4), и выдавать его за
    # существующий нельзя. Демо-номер моделирует номер, где он есть.
    # Четвёртая сцена — «Чтение» из макета: сцены на объекте есть, их число
    # разное от номера к номеру, и демо-номер моделирует тот, где их четыре.
    "305": {"lights": 5, "curtains": 2, "fcus": 1, "scenes": 4},
}
# Комнаты, за которые эмулятор отвечает.
#
# Два разных источника, и смешивать их не надо:
#
# 1. КОМНАТЫ С ОБЪЕКТА — те, что были прозвонены. У 701/706/708 профили каналов
#    сняты с настоящего оборудования (см. ROOM_PROFILES), и трогать их нельзя:
#    на них проверяют, что мы умеем работать с реальной, а не удобной нам
#    раскладкой.
#
# 2. КОМНАТЫ ДЕМО-ОТЕЛЕЙ — чтобы гость на стенде мог открыть ЛЮБОЙ номер, а не
#    угаданный. Раньше набор пересекался с «Кристаллом» лишь на четырёх номерах
#    (201, 301, 305, 401), и остальные пять показывали «обратитесь на ресепшен»:
#    оборудования за ними не было вовсе. На демо это выглядит поломкой продукта,
#    хотя сломана оснастка.
#
#    Формы взяты те же, что в сидах, а не переписаны числами: разъедутся —
#    видно будет по форме, а не по одному пропавшему номеру.
#       «Кристалл»  seed_demo_hotel:  этажи 2–4 × комнаты 01, 05, 12
#       «Азур»      seed_demo_fleet:  1-й этаж 101–108, 2-й 201–206
#       «Люмен»     seed_demo_fleet:  01–12
#
# Номер комнаты — единственное, чем различаются устройства (`DEVICE_TEMPLATE`),
# поэтому одинаковые номера в разных отелях делят одно устройство эмулятора.
# Для стенда это безвредно: отели показывают разные витрины, а не разное железо.
_SURVEYED_ROOMS = ["202", "203", "415", "501", "601", "701", "706", "708"]
_CRYSTAL_ROOMS = [f"{floor}{index}" for floor in ("2", "3", "4") for index in ("01", "05", "12")]
_AZURE_ROOMS = [f"1{n:02d}" for n in range(1, 9)] + [f"2{n:02d}" for n in range(1, 7)]
_LUMEN_ROOMS = [f"{n:02d}" for n in range(1, 13)]

DEFAULT_ROOMS = sorted(
    set(_SURVEYED_ROOMS) | set(_CRYSTAL_ROOMS) | set(_AZURE_ROOMS) | set(_LUMEN_ROOMS)
)
