...
# КАДРИРОВАННЫЙ рендер, а не исходный.
#
# У исходного PNG комната занимала около 62% ширины, остальное — дорисованный
# тёмный фон. Карточка плиты при этом была ровно по картинке (соотношение
# сходилось до третьего знака), и «пустые поля» вокруг плана были не вёрсткой,
# а частью самого кадра: вёрсткой это вылечить было нечем.
#
# Поля срезаны, соотношение сторон изменилось с 1.599 на 1.056 — плита стала
# почти квадратной. Пропорция приезжает в payload.plan и берётся оттуда, в коде
# её нет: тип номера с другим рендером подключается заменой файлов, а не правкой
# компонента.
GEOMETRY_FILE = CONCEPT_DIR / "plan-geometry-cropped.json"
RENDER_FILE = CONCEPT_DIR / "render-type1-cropped.png"
...
#
# v2: у прежнего кадра светильники и светодиодная лента ОСТАВАЛИСЬ ГОРЯЩИМИ.
# Деление на освещённость убирает разлив света, тени и перепад «под лампой
# ярче», но выключить сам источник не может: его пиксели ярки собственным
# свечением, и после деления материал читается как белый. Скрипт получил
# отдельный проход, который заменяет источники размытым окружением.
RENDER_OFF_FILE = CONCEPT_DIR / "render-type1-cropped-off-v2.png"
