#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# Единая точка входа контейнера. Режимы: api | worker | beat | scheduler | shell
# ---------------------------------------------------------------------------
set -euo pipefail
...
case "$MODE" in
    api)
        ...
        ;;
    worker)
        # ЧИСЛО ПРОЦЕССОВ — НАСТРОЙКА, А НЕ УМОЛЧАНИЕ CELERY. Без неё Celery
        # берёт число ядер: на восьмиядерной машине это одиннадцать процессов
        # и 728 МиБ под очередь, которая на стенде почти пуста (замер
        # 18.09.2026). Два процесса держат ~300 МиБ и по-прежнему дают
        # параллельность: одна задача не блокирует остальные.
        #
        # Здесь, а не в compose: файлов compose два (dev и prod), и число
        # процессов обязано быть ОДНИМ ответом для обоих. Нужно больше —
        # поднимается переменной, без правки кода.
    ...
    beat)
        ...
        ;;
    scheduler)
        # СЛУЖБА ОТЛОЖЕННОГО ЗАПУСКА. Источник правды — таблица назначенных
        # заданий; служба лишь регулярно спрашивает, чему пришёл срок. Поэтому
        # она не хранит состояния, и её можно перезапускать когда угодно:
        # пропущенное она подхватит и честно пометит опоздание.
        exec python manage.py run_scheduler --interval "${SCHEDULER_INTERVAL:-60}"
        ;;
    *)
        exec "$@"
        ;;
