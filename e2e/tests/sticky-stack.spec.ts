...
test('высота документа не меняется на прокрутке экрана номера', async ({ page }) => {
  /*
    Требование из G5d остаётся, а причина изменилась.

    Раньше высоту документа могло увести СЖАТИЕ ПЛИТЫ: место под липкий план
    было зарезервировано, а картинка ужималась трансформом. Плита больше не
    липкая и не сжимается — но правило то же: поехавшая высота документа
    заставляет браузер поправлять позицию прокрутки, и экран трясёт. Здесь и
    проверяется, что теперь её не ведёт НИЧЕГО.
  */
...
test('вкладки номера прилипают, а план уезжает над ними', async ({ page }) => {
  /*
    БЫЛО: «вкладки идут под плитой на всём пути прокрутки» — проверялось, что
    зазор между низом плиты и верхом вкладок неотрицателен. С неподвижным
    планом это утверждение стало НЕОПРОВЕРЖИМЫМ: план уезжает вверх, зазор
    только растёт, и тест зеленел бы при любой поломке липкости.

    СТАЛО: проверяется то, что теперь и есть правило экрана. План уходит
    ВЫШЕ вкладок и за верх экрана — то есть он в потоке; вкладки при этом
    остаются в верхней части экрана — то есть они липкие.
  */
  await page.setViewportSize({ width: 390, height: 844 })
  await enterRoom(page, 'dark')
  await page.goto('/room')
  ...
  const geometry = () =>
    page.evaluate(() => {
      ...
      return plate && tabs ? { plateBottom: plate.bottom, tabsTop: tabs.top } : null
    })

  const atRest = await geometry()
  expect(atRest, 'плита или вкладки не найдены').not.toBeNull()
  ...
  for (const y of [0, 40, 90, 140, 200, 320, 500, 800]) {
    ...
    expect(
      Math.round(now!.tabsTop - now!.plateBottom),
      `на скролле ${y} вкладки заехали под план`,
    ).toBeGreaterThanOrEqual(0)
    /*
      ВНИЗ ВКЛАДКИ НЕ ЕДУТ НИКОГДА.

      Порога «выше 400px на любой прокрутке» здесь стоять не может: липкость
      включается не сразу, а когда полоса ДОЕДЕТ до своего края. На сорока
      пикселях она ещё идёт с потоком и честно стоит на 426 — и прежний порог
      ловил именно это, а не поломку.
    */
    expect(
      Math.round(now!.tabsTop),
      `на скролле ${y} вкладки уехали ВНИЗ от места покоя`,
    ).toBeLessThanOrEqual(Math.round(startTop) + 2)
  }

  // А ДОЕХАВ — стоят вверху экрана: на глубокой прокрутке липкость обязана
  // быть уже включена, иначе полоса ушла бы за верх вместе со страницей.
  const pinned = await geometry()
  expect(pinned!.tabsTop, 'на глубокой прокрутке полоса вкладок не прилипла').toBeLessThan(400)
  expect(pinned!.tabsTop, 'полоса вкладок ушла за верх экрана').toBeGreaterThan(0)

  // План действительно ушёл наверх, а не остался приклеенным.
  const deep = await geometry()
  expect(deep!.plateBottom, 'план не уехал за верх экрана — он всё ещё липкий').toBeLessThan(0)
})
 ...
 * на тапы не отвечает. Тогда виновата была липкая обёртка плиты: плита
 * сжималась трансформом, раскладку трансформ не меняет, обёртка оставалась
 * прежней высоты и, стоя выше по z, забирала нажатие себе. Ни сжатия, ни
 * липкой обёртки больше нет — план листается вместе со страницей.
 *
 * Проверка остаётся, и вопрос у неё тот же: не «видно ли вкладку», а КТО
 * получит нажатие в её середине. Над полосой по-прежнему есть плавающая
 * группа, и геометрия соседей такой перехват пропускает — перекрытия по
 * прямоугольникам может не быть вовсе.
 */
test.describe('вкладки номера ловят нажатие сами', () => {
  /*
    ТОЛЬКО УЗКИЕ ШИРИНЫ, И ЭТО ПРАВИЛО, А НЕ ПРОБА.

    На ширине от 1024px (`DESKTOP_QUERY`) экран номера раскладывается в две
    колонки БЕЗ вкладок вовсе: прятать половину управления за переключателем
    там незачем. Раньше это выражал `test.skip(count === 0)` — то есть решение
    раскладки проверялось на ощупь, и тот же `skip` молчал бы, если бы вкладки
    пропали там, где обязаны быть. Ширина названа явно: где вкладок нет по
    устройству — тест не идёт, где есть — их отсутствие это падение.
  */
  for (const view of VIEWPORTS.filter((v) => v.width < 1024)) {
    for (const mode of ['dark', 'light'] as const) {
      test(`нажатие достаётся вкладке: ${view.name}, ${mode}`, async ({ page }) => {
        ...
        // экран номера сломан и вкладки не отрисовались вовсе.
        const tabs = page.locator('[role="tab"]')
        await expect(tabs.first(), 'у номера не отрисовались вкладки').toBeVisible({
          timeout: 15_000,
        })
        const count = await tabs.count()
        expect(count, 'вкладок меньше двух — переключать нечего').toBeGreaterThan(1)
