...
/*
  ПОТОЛОК ТЕСТА ВЫШЕ ОКНА ПЕРЕПОДКЛЮЧЕНИЯ. Доска живёт вебсокетом, и оборванное
  соединение восстанавливается с нарастающей паузой до 30 секунд; на
  переподключении сервер присылает полный снимок, то есть заказ доедет — но
  ожидание внутри теста при этом занимает почти весь шестидесятисекундный
  потолок, и падал уже сам тест, а не проверка. Двух минут хватает и на
  переподключение, и на остальные шаги.
*/
