...
test('десять параллельных запросов на протухшем access — одно обновление', async ({
  ...
  /*
    ПАУЗА ЗАКОННА И ОСТАЁТСЯ: здесь ждут ВРЕМЯ, а не событие.

    Токен живёт заданное число секунд, и «истёк» — свойство часов, а не
    состояние экрана. Условия, по которому можно было бы ждать, не существует:
    в момент истечения сервер ничего не присылает.
  */
