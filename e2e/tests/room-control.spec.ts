...
test.describe('План: обводка зоны', () => {
  ...
  test('тап пальцем: обводка держится, пока идёт команда, и уходит с исходом', async ({
    ...
    */
    ...
    /*
      ПОЛЁТ КОМАНДЫ ДЕЛАЕМ УПРАВЛЯЕМЫМ, а не надеемся на скорость сети.

      Проверка звучит «обводка держится, ПОКА идёт команда», то есть говорит о
      состоянии внутри окна между отправкой и ответом. Тест читал обводку
      отдельным запросом в браузер сразу после ожидания `aria-busy`, и на
      быстром ответе команда успевала завершиться между этими двумя обращениями:
      обводки уже нет — «команда идёт, а обводки нет». В одиночку проходило, в
      пакете падало, и виноватой выглядела разметка.

      Задержка ответа делает окно заведомо шире любого промежутка между двумя
      обращениями к странице. Это не «подкрутить таймаут»: мы не ждём дольше, мы
      удерживаем ровно то состояние, о котором утверждаем.
    */
    await page.route('**/guest/room/command', async (route) => {
