...
test.describe('Главная: погода и время', () => {
  ...
  test('погода есть — градусы, состояние НАШИМ словом и атрибуция провайдера', async ({
    page,
  }) => {
    ...
    /*
      АТРИБУЦИЯ — УСЛОВИЕ ЛИЦЕНЗИИ, а не подпись для красоты. Проверяется и её
      наличие, и что она ведёт к провайдеру: подпись без ссылки условие не
      выполняет.
    */
    const credit = page.getByTestId('guest-home-weather-attribution')
