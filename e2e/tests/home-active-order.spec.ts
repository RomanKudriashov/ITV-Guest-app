...
async function enterAsGuest(page: Page, room = DEMO_ROOM): Promise<void> {
  ...
  // После входа гость попадает на главную-витрину; в каталог ведёт openMenu.
...
test('полоса активного заказа: статус, время подачи, живое обновление', async ({ page, request }) => {
  ...
  // Витрина сервисов на месте — главная это bento, а не список быстрых
  // действий (ряд действий-сирот убран в пользу плиток).
