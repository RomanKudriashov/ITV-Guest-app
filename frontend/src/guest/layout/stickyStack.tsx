 ...
 *
 * ИЗМЕРЯЕТСЯ ВИДИМАЯ ГЕОМЕТРИЯ, а не layout-высота: `getBoundingClientRect()`
 * возвращает именно её, и слой ниже отступает ровно на то, что видно.
 *
 * `scale` и `publish` СЕЙЧАС НИКТО НЕ ЗОВЁТ, и это надо знать, читая их ниже.
 * Они появились ради единственного случая: плита плана номера сжималась при
 * скролле трансформом, `ResizeObserver` таких изменений не видит вовсе, и она
 * публиковала свой видимый размер сама. Сжатия больше нет — на телефоне план
 * листается вместе со страницей и место освобождает сам, — но механика
 * оставлена: она про ЛЮБОЙ слой, меняющий видимый размер трансформом, а такой
 * появится снова раньше, чем её успеют написать заново.
 */
