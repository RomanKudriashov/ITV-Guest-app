 ...
 *
 * `isReady: true` — И ЭТО НЕ ВРАНЬЁ, А ЕДИНСТВЕННЫЙ ЧЕСТНЫЙ ОТВЕТ.
 *
 * Оболочка витрины уводит на экран входа, пока флаг не поднят
 * (`GuestLayout`: `if (!isReady) return <Navigate to="/" />`). Ответив «не
 * готова», показ показывал бы ровно один экран — вход — какой бы ни выбрали в
 * переключателе. Флаг отвечает на вопрос «можно ли рисовать витрину», а не
 * «есть ли гость»: рисовать можно, гостя нет, и про гостя честно сказано
 * ниже — `session: null`, `canOrder: false`.
 *
 * ГОСТЕВЫХ ЗАПРОСОВ ПРИ ЭТОМ НЕ УХОДИТ НИ ОДНОГО. Клиент запросов показа
 * отказывается ходить в сеть вовсе (`usePreviewData`): данные положены в кэш
 * серверной ручкой панели, а всё, чего там нет, честно падает в состояние
 * ошибки вместо похода в гостевую ручку без токена.
 *
