...
export function RoomPage() {
  ...
  */
  ...
  /*
    ПЛИТА БОЛЬШЕ НЕ ЗАНИМАЕТ ПОЛОСУ СТЕКА — НИ НА ОДНОЙ ШИРИНЕ.

    На телефоне план листается ВМЕСТЕ СО СТРАНИЦЕЙ. Липкий план съедал треть
    невысокого экрана всё время, пока человек ищет нужный тумблер, а нужен он
    ровно в момент выбора комнаты — то есть в самом верху. Сжатие на прокрутке
    было попыткой вернуть это место, но плата за него — дрожание всей верхушки
    и тот самый «наезд слоёв друг на друга».

    На широком экране план остаётся слева и остаётся липким — но там он стоит
    В СВОЕЙ КОЛОНКЕ, ничего собой не закрывает, и место у списка справа не
    отнимает вовсе. Липким его делает собственный `position: sticky` в колонке,
    а не общий стек: под ним нет слоёв, которым он должен сообщать свою полосу.

    Слой остаётся объявленным ради `plateLayer.top` — верхнего края под шеллом:
    по нему считается и отступ страницы, и `top` липкой колонки десктопа.
  */
  const plateLayer = useStickyLayer<HTMLDivElement>(STICKY.plate, { enabled: false });
  // Вкладки — единственный липкий слой узкого экрана, и пинятся они прямо под
  // плавающей группой: полосы плиты над ними больше нет.
  ...
  const plate = snapshot?.plan ? (
    <RoomPlanPlate
      ...
      /*
        ...
        На десктопе плита стоит в своей колонке рядом со списком, ничего
        собой не закрывает, и пропорция кадра там как раз закрыла пустоту под
        планом. А на узком экране плита стоит НАД управлением, и её высота
        прямо отнимается у того места, где живут контролы.
      */
      maxHeight={isDesktop ? undefined : layout.planMaxHeightNarrow}
      ...
      {isDesktop ? (
        ...
        */
        ...
        <>
          {plate ? (
            /*
              ПЛАН ЛИСТАЕТСЯ ВМЕСТЕ СО СТРАНИЦЕЙ.

              Был `position: sticky` — и держал треть невысокого экрана всё
              время, пока человек ищет тумблер в списке под ним. Нужен он в
              момент выбора комнаты, то есть в самом верху; дальше он мешает.

              Вместе с липкостью ушли две подпорки, которые только ею и
              объяснялись: сжатие плиты трансформом на прокрутке (оно
              освобождало место, которое теперь освобождается само) и
              `pointerEvents: none` на обёртке — обёртка не сжимается и больше
              не накрывает собой вкладки, так что перехватывать у неё указатель
              не нужно.
            */
            <Box
              sx={{
          ...
          {tabs.length > 1 && !unavailable ? (
            <Box
              ...
              sx={{
                ...
                zIndex: 1,
                /*
                  ВОЗДУХ НАД ПАНЕЛЬЮ — ТОЛЬКО В НЕПРИЛИПШЕМ СОСТОЯНИИ.

                  В прилипшем его быть не должно: щель между двумя липкими
                  слоями прозрачна, и в неё на прокрутке заглядывают строки
                  списка. Поэтому у плиты `gap: 0`, панель встаёт вплотную, а
                  своё дыхание отдаёт внутренним отступом на плотном фоне.

                  В покое же панель начиналась ровно на нижней границе кадра —
                  её фон касался плана, и вкладки читались приклеенными.
                */
                mt: 1.25,
                /*
                  ОТБОЙ СНИЗУ — СОРАЗМЕРНЫЙ, а не «наезд панели».

                  Здесь стоял `panelOverlap` (28) — число из каталога, где им
                  панель НАЕЗЖАЕТ скруглением на кадр. Наезда тут нет, и
                  двадцать восемь пикселей просто складывались с отступами
                  ленты: сверху вкладки прилипали к плану на девять пикселей,
                  снизу до первого инструмента зияло девяносто семь.
                */
                mb: 1.5,
                ...
                // языка.
                px: 1.5,
                // Внутри блока воздух СИММЕТРИЧЕН: полоса стоит по центру
                // своей панели, а не подпирает её верхний край.
                py: 1.25,
          ...
          {/*
            Плашки — ПОД полосой вкладок, а не над ней.

            Стояли выше и вставали МЕЖДУ двумя липкими слоями: появившись,
            плашка сдвигала полосу вкладок вниз на свою высоту. На телефоне это
            читается как «управление уехало из-под пальца» — а появляется она
            ровно в ответ на нажатие, то есть каждый раз, когда палец там и
            находится. Под вкладками она не двигает ничего, кроме содержимого
            ленты, которое и так листается.
          */}
          {/* Контейнер появляется ВМЕСТЕ с плашками: пустой он всё равно
              занимал свои шестнадцать пикселей, и они уходили в тот самый
              провал между вкладками и инструментами. */}
          {notices ? <Stack sx={{ mt: 2 }}>{notices}</Stack> : null}
          ...
          {tabs.length > 1 && !unavailable ? (
            ...
            >
              {/* Отступ ленты СОРАЗМЕРЕН воздуху над полосой вкладок: панель
                  стоит между кадром и инструментами, и обе стороны у неё
