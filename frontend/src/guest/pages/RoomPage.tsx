          ...
          {plate ? (
            <Box
              sx={{
                ...
                /*
                  УКАЗАТЕЛЬ ЛОВИТ ТОЛЬКО ВИДИМАЯ ПЛИТА.

                  Плита сжимается трансформом, а раскладку трансформ не трогает:
                  этот блок остаётся ПРЕЖНЕЙ высоты и, стоя выше по z, накрывает
                  собой вкладки. На прокрутке они видны — и не отвечают на тап:
                  нажатие достаётся пустому месту обёртки.

                  Высоту здесь сжимать нельзя: она держит высоту документа, и от
                  её пересчёта экран начинает трястись (сторож рядом это пинит).
                  Поэтому обёртка становится прозрачной для указателя, а обратно
                  его включает сама плита — ровно та область, которую видно.
                */
                pointerEvents: 'none',
                '& [data-testid="room-plan"]': { pointerEvents: 'auto' },
              }}
            >
