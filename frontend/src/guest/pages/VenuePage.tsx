...
export function VenuePage() {
  ...
  return (
    <Box data-testid="guest-venue" data-venue-type={venue.type} data-content={content}>
      ...
      {/*
        ЗАВЕДЕНИЕ ЗАКРЫТО — СКАЗАТЬ ЭТО СТРОКОЙ, А НЕ ПИЛЮЛЕЙ В УГЛУ.

        Часы в шапке были, но мелким значком поверх кадра: гость доходил до
        меню, не понимал, почему у блюд нет кнопок, и решал, что приложение
        сломалось. Здесь сказано и что закрыто, и когда откроется, и — главное
        — что смотреть при этом можно.

        Плашка стоит ВЫШЕ каталога и ниже кадра, тем же приёмом, что и
        «только просмотр»: нахлёст панели каталога съедает нижнюю строку, если
        отдать ему текст вместо пустого места.
      */}
      {venue.is_open === false ? (
        <Box
          sx={{
