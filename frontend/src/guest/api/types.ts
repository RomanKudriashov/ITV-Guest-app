...
export interface GuestHome {
  hotel: {
    ...
    timezone?: string;
    /** Город — подпись к погоде и часам на языке гостя. Пусто — подписи нет. */
    city?: string;
  };
  room: string | null;
...
export interface RoomPlan {
  ...
  image_off: string;
  /**
   * ВИД экрана: `tiles` сюда не приходит вовсе (у плашек плана нет).
   *
   * Гость про уровни не знает и слова этого не видит — поле говорит плите, КАК
   * рисовать комнату: простой план обводит зоны и заливает включённые, полный
   * показывает свет настоящим светом. Выводить это из `image_off` значило бы
   * вернуть догадку, ради устранения которой уровень и заведён.
   */
  level?: 'simple' | 'full';
  aspect: number | null;
  /**
