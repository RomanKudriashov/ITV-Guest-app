...
/**
 * Порог для ТЕКСТА — WCAG 1.4.3 (AA): 4,5:1.
 *
 * Отдельно от `NON_TEXT_CONTRAST`, потому что это разные требования к разным
 * вещам: 3:1 хватает заливке и рамке, буквам — нет.
 */
export const TEXT_CONTRAST = 4.5;

/**
 * Во сколько раз различимы два цвета. Нужна не только витрине: редактор бренда
 * обязан сказать оператору, что выбранный цвет текста нечитаем, и мера у них
 * должна быть ОДНА — иначе редактор разрешит то, на чём витрина сломается.
 */
export function contrastRatio(a: string, b: string): number {
  return contrast(rgbOf(a), rgbOf(b));
}
