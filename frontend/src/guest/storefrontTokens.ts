...
export interface StorefrontTokens {
  ...
  roomPlan: {
     ...
     * форма прозрачности — чёрный здесь означает «показать», а не «покрасить».
     * Растушёвка обязательна: свет в комнате не обрывается по прямой.
     */
    zoneWindow: string;
...
const ROOM_PLATE = {
  ...
  zoneWindow:
    'radial-gradient(ellipse at center, #000 0%, #000 46%, transparent 82%)',
