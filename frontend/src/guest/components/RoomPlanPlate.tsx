...
/**
 * Сколько кадра плита обязана показывать, упершись в потолок высоты.
 *
 * Обрезка — размен, и у него есть дно. Ниже примерно шести седьмых кадра
 * комната перестаёт читаться комнатой: пропадает нижний ряд помещений, а с ним
 * и всё, что на нём нарисовано, — метки повисают половинками на самом обрезе.
 * Телефон в альбомной ориентации требовал бы оставить от кадра 39%, то есть
 * верхнюю полосу с двумя лампами из пяти.
 *
 * Дойдя до дна, потолок высоты перестаёт обрезать и начинает СУЖАТЬ плиту: она
 * больше не во всю колонку, зато кадр снова помещается целиком. Запас до самой
 * нижней метки берётся с ЗАПАСОМ, потому что метка — кружок постоянного
 * размера в пикселях: на маленькой плите её половина занимает вдвое большую
 * долю высоты, чем на большой, и дно, посчитанное по крупной, на мелкой уже
 * режет. Правило про сам кадр, а не про планировку, поэтому число живёт здесь,
 * а не в конфигурации типа номера.
 */
const MIN_FRAME_SHOWN = 0.85;
  ...
  return (
    <Box
      ...
      style={{
        ...
        maxHeight,
        // …но не глубже дна обрезки. Дошли до него — потолок начинает сужать
        // плиту, и кадр снова помещается в неё целиком: ширина, при которой
        // от кадра остаётся ровно `MIN_FRAME_SHOWN`. На телефоне в портрете
        // эта ширина больше колонки и потому ничего не меняет.
        // `min(100%, …)` обязателен: инлайновый стиль перебивает `maxWidth`
        // из sx целиком, и без него плита в широкой колонке вылезла бы за неё.
        maxWidth: maxHeight
          ? `min(100%, calc(${maxHeight} * ${(plan.aspect / MIN_FRAME_SHOWN).toFixed(3)}))`
          : undefined,
