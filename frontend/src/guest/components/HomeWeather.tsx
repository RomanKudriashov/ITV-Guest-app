 ...
 *
 * АТРИБУЦИЯ ОБЯЗАТЕЛЬНА — И ЭТО НЕ УКРАШЕНИЕ.
 *
 * Данные Open-Meteo распространяются под CC BY 4.0, и указание источника
 * требуется при ЛЮБОМ способе использования: и на бесплатном плане, и на
 * платном, и на своём экземпляре — потому что сами данные приходят из чужих
 * источников (DWD, ECMWF и другие).
 *
 * Подпись снимали по указанию владельца продукта, и всё это время условие
 * лицензии не выполнялось. 21.09.2026 решение пересмотрено на встрече:
 * подпись возвращена — аккуратно, последней строкой блока, мелким
 * второстепенным текстом. Убирать её снова нельзя без смены провайдера на
 * того, чьи условия подписи не требуют. Порядок — docs/ops/weather.md.
 */
  ...
  return (
    ...
    >
      ...
      */}
      ...
      {/*
        Источник данных. Стоит ПОСЛЕДНИМ и мельче остального: это выполнение
        лицензии, а не сообщение гостю. Показывается только когда погода
        действительно есть: подпись под пустым блоком — подпись ни к чему.
      */}
      {weather ? (
        <Typography
          variant="caption"
