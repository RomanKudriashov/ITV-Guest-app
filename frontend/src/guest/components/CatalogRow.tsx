    ...
    >
                ...
                */
                sx={(theme) => ({
                  ...
                  /*
                    ...
                    Здесь стояло `minHeight: 32` — место под две строки всегда.
                    На демо-меню однострочны ВСЕ восемнадцать описаний, и под
                    каждым висела пустая строка: «Мраморная говядина, 300 г,
                    гриль» занимает строку, а стеклянная подложка была вдвое
                    выше текста и читалась незакрытой.

                    Запас держали ради ровного ряда — и напрасно: карточки
                    равняет сетка. Каждая растянута на высоту ряда
                    (`height: 100%`), а ряд берёт высоту по самой высокой
                    карточке; цена с кнопкой прижаты к низу через `mt: auto`.
                    Измерено: до правки расхождение высот в ряду было 0 px и
                    после — тоже 0.

                    Обрезка по второй строке ОСТАЁТСЯ. Она не про выравнивание,
                    а про защиту от длинного описания: одно на десять строк
                    вытянуло бы весь ряд.
                  */
                  display: '-webkit-box',
                  WebkitLineClamp: 2,
      ...
      <Box sx={{ px: '14px', pb: '14px', pt: description ? 1 : '13px', display: 'flex', flexDirection: 'column', flexGrow: 1 }}>
        ...
        <Stack
          ...
          spacing={1.25}
          /*
            Нижняя строка держит высоту, только когда ей есть что держать.
            Безусловные 40 пикселей оставляли пустую полосу под карточкой
