  ...
  return (
    <Box
      ...
      sx={{
        overflow: 'hidden',
        /*
          ВЕРТИКАЛЬ — БРАУЗЕРУ, ГОРИЗОНТАЛЬ — НАМ.

          Без этого правила у ленты `touch-action: auto`, то есть браузер
          вправе забрать ЛЮБОЙ жест себе: пока он решает, прокрутка это или
          нет, события до обработчика идут рвано, а на iOS горизонтальная
          протяжка у края экрана и вовсе уходит в жест «назад». Панель при
          этом дёргается вместо того, чтобы ехать за пальцем.

          `pan-y` оставляет браузеру вертикальную прокрутку страницы —
          вертикальный жест лента и так не забирает, это её собственное
          правило, — и отдаёт горизонталь обработчику сразу, без раздумий.

          ВАЖНО: причина НЕ подтверждена на устройстве. Замер показал, что при
          доставленных событиях лента едет за пальцем один к одному (9 кадров
          из 9), то есть ломается не логика, а доставка. Правка сделана по
          механизму и ждёт проверки на телефоне.
        */
        touchAction: 'pan-y',
      }}
    >
