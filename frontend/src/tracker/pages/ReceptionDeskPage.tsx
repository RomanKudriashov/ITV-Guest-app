...
export function ReceptionDeskPage() {
  ...
  const conversation = selected ? (
    <DeskConversation key={selected} threadId={selected} myId={user?.id} />
  ) : (
  ...
  return (
    <Box data-testid="reception-desk" sx={{ height: 'calc(100dvh - 64px)', display: 'flex', flexDirection: 'column' }}>
          ...
          {!selected ? (
            ...
            <>
              <Stack direction="row" alignItems="center" sx={{ px: 1 }}>
                <Button size="small" startIcon={<ArrowBackIcon />} onClick={() => open(null)} data-testid="desk-back">
                  {t('tracker.chat.back')}
                </Button>
                <Tabs value={pane} onChange={(_e, next) => setPane(next)} sx={{ ml: 'auto' }}>
                  <Tab value="chat" label={t('tracker.desk.chatTab')} />
                  <Tab value="guest" label={t('tracker.desk.guestTab')} data-testid="desk-guest-tab" />
                </Tabs>
              </Stack>
              <Box sx={{ flexGrow: 1, minHeight: 0, overflowY: pane === 'guest' ? 'auto' : 'hidden' }}>
