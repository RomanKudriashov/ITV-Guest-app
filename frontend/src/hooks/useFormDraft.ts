...
export function useFormDraft<T>({
  ...
  /*
    ОБЪЁКТ ЧЕРНОВИКА — СТАБИЛЬНЫЙ, И ЭТО НЕ ОПТИМИЗАЦИЯ.

    Три функции внутри уже мемоизированы, но `{ save, restore, discard }`
    создавался заново на КАЖДОМ рендере. Экран кладёт этот объект в
    зависимости эффекта — и эффект срабатывал каждый рендер. В редакторе
    позиции такой эффект выставляет baseline, то есть меняет состояние: рендер
    → эффект → состояние → рендер, и React упирался в «Maximum update depth
    exceeded».

    Снаружи это выглядело так: экран редактора живёт, а переход по меню меняет
    ТОЛЬКО АДРЕС — перерисоваться приложение уже не успевает. Жалоба клиента
    звучала как «сайдбар не работает», а сломан был черновик.
  */
  return useMemo(() => ({ save, restore, discard }), [save, restore, discard]);
}
