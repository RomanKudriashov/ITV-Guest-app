...
/**
 * Слой поверх ФОТОГРАФИИ — общий для всех поверхностей.
 *
 * Режима не касается намеренно: под этим слоем всегда кадр, а не фон страницы,
 * и подпись на нём белая в любой теме. «Осветлить ради светлой темы» здесь
 * значит сделать подпись нечитаемой ровно на светлых кадрах.
 *
 * Живёт в платформенном словаре, а не в словаре витрины: этим же слоем
 * пользуется общий кит карточек, а киту нельзя зависеть от гостя.
 */
export const MEDIA_OVERLAY = {
  /** Сильный вертикальный скрим карточки: подпись читается на любом кадре. */
  cardScrim:
    'linear-gradient(180deg, rgba(0,0,0,0.08) 34%, rgba(0,0,0,0.52) 72%, rgba(0,0,0,0.86) 100%)',
  /** Тень под подписью поверх кадра. */
