...
> **Статус: срез «еда» замкнут.** Есть каркас, CMS-раздел «Меню» (каталог,
> модификаторы, фото, расписания) и гостевая витрина: вход по QR, меню,
> заказ и живой статус. Не хватает трекера персонала — заказы двигаются
> эндпоинтом `POST /api/orders/{id}/status`, UI доски будет следующим
> прогоном.
...
* UI трекера персонала — следующий прогон. Плюмбинг готов: смена статуса
